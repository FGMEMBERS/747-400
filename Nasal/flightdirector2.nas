...
INIT = func {
    ...
    setprop("/instrumentation/flightdirector/bar-pitch", 0.0);
    setprop("/instrumentation/flightdirector/bar-roll", 0.0);
...
#############################################################################
# update the FD bar position
#############################################################################

update_bar = func {

        tgtrad = getprop("/autopilot/settings/true-heading-deg");
        if ( tgtrad == nil or tgtrad == "" ) {
            tgtrad = 0.0;
        }
        curhdg = getprop("/orientation/heading-deg");
        diff = tgtrad - curhdg;
        if ( diff < -180.0 ) {
            diff += 360.0;
        } elsif ( diff > 180.0 ) {
...
vnav = getprop("/instrumentation/flightdirector/vnav");
if (vnav != 0 or vnav !=nil){bar_pitch =  get_altpitch();}

lnav_last = lnav;
bar_pitch_prop = (bar_pitch - getprop("/orientation/pitch-deg"));
bar_roll_prop = (getprop("/orientation/roll-deg") - bar_roll);
if(bar_roll_prop > 30.0){bar_roll_prop = 30.0;}
if(bar_roll_prop < -30.0){bar_roll_prop = -30.0;}
if(bar_pitch_prop > 15.0){bar_pitch_prop = 15.0;}
if(bar_pitch_prop < -15.0){bar_pitch_prop = -15.0;}

setprop("/instrumentation/flightdirector/bar-pitch", bar_pitch_prop);
setprop("/instrumentation/flightdirector/bar-roll", bar_roll_prop);
